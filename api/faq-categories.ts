...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (method === "GET") {
    const response = await fetch(
      `${supabaseUrl.replace(/\/$/, "")}/rest/v1/faq_categories?select=id,name,sort_order,created_at,updated_at&order=sort_order.asc,name.asc`,
      {
        method: "GET",
        headers,
      },
  ...
  if (method === "DELETE") {
    ...
    const response = await fetch(
      `${supabaseUrl.replace(/\/$/, "")}/rest/v1/faq_categories?id=eq.${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        headers,
      },
