...
import { verifyPasswordResetToken } from './password-reset-utils';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const updateResponse = await fetch(`${SUPABASE_URL.replace(/\/$/, '')}/auth/v1/admin/users/${payload.sub}`, {
      method: 'PUT',
      headers: {
    ...
    try {
      const sendEmailResponse = await fetch(`${confirmationBaseUrl.replace(/\/$/, '')}/api/send-email`, {
        method: 'POST',
      ...
      if (!sendEmailResponse.ok) {
        ...
        console.warn('Password change confirmation email failed', sendEmailResponse.status, sendEmailText);
      }
    } catch (emailError) {
    ...
    return res.status(400).json({ error: error.message || 'Invalid token or password' });
  }
}
