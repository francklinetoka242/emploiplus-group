...
if (!recipient) {
  console.error(
    "Usage: node scripts/test-smtp.js recipient@example.com (or set TEST_RECIPIENT env var)",
  );
  process.exit(1);
}
...
if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
  console.error(
    "Missing SMTP environment variables. Ensure SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS are set.",
  );
  process.exit(2);
}
