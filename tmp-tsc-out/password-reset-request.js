...
export default async function handler(req, res) {
    ...
    try {
        const candidateResponse = await fetch(`${SUPABASE_URL.replace(/\/$/, '')}/rest/v1/candidates?select=user_id,email&email=eq.${encodeURIComponent(email)}`, {
            method: 'GET',
            headers: {
        ...
        const candidateBody = await readResponseBody(candidateResponse);
