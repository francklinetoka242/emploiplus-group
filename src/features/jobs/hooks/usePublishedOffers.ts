...
export function usePublishedJobOffers(limit = 10) {
  ...
  React.useEffect(() => {
    ...
    async function fetchOffers() {
      const offers = await jobService.getPublishedOffers(limit);
      if (!mounted) return;
      setOffers(offers as JobOfferPreview[]);
      setLoading(false);
    }
...
export function usePublishedBlogPosts(limit = 9) {
  ...
  React.useEffect(() => {
    ...
    async function fetchPosts() {
      ...
      ).eq("status", "published").order("is_featured", { ascending: false }).order("sort_order", { ascending: true }).order("publish_at", { ascending: false }).limit(limit);
