...
import { getCandidatePreferences, saveCandidatePreferences } from "@/features/candidates/api/preferencesApi";
...
export function useCandidatePreferences(candidateId?: string | null) {
  ...
  const fetchPreferences = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getCandidatePreferences(candidateId);
      setPreferences(data);
    } catch (err) {
  ...
  const savePreferences = useCallback(async (values: CandidatePreferencesInsert) => {
    ...
    const saved = await saveCandidatePreferences(candidateId, values);
