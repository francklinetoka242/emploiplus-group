  ...
  if (!inflight) {
    inflight = fetch("https://ipapi.co/json/", { headers: { Accept: "application/json" } })
      .then(async (response) => {
        ...
        const payload = (await response.json()) as Partial<IpApiResponse> | null;
...
>(jobs: T[], geo: Geo | null): T[] {
  const now = Date.now();
  const score = (j: T) => {
    let s = 0;
    if (j.featured_until && new Date(j.featured_until).getTime() > now) s += 1000;
    if (geo?.city && j.location_city && j.location_city.toLowerCase() === geo.city.toLowerCase())
      s += 500;
    if (
      geo?.country_name &&
      j.location_country &&
      j.location_country.toLowerCase() === geo.country_name.toLowerCase()
    )
      s += 200;
...
export function jobLocBadge<
  T extends { location_city?: string | null; location_country?: string | null },
>(j: T, geo: Geo | null): "near" | "country" | null {
  if (!geo) return null;
  if (geo.city && j.location_city && j.location_city.toLowerCase() === geo.city.toLowerCase())
    return "near";
  if (
